...
local requestsDisabled = true --getgenv and getgenv().DISABLE_ALICEHUB_REQUESTS
local InterfaceBuild = 'ShiroKanaeru'
local Release = "Build 1.0"
local AliceHubFolder = "AliceHub"
local ConfigurationFolder = AliceHubFolder.."/Configurations"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(AliceHubFolder) then
				if isfile and isfile(AliceHubFolder..'/settings'..ConfigurationExtension) then
					file = readfile(AliceHubFolder..'/settings'..ConfigurationExtension)
				end
			end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("AliceHub", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify ShiroKanaeru developers at t.me/shirostoree")
				end
			end)
...
if AliceHub.AbsoluteSize.X < minSize.X and AliceHub.AbsoluteSize.Y < minSize.Y then
...
local Main = AliceHub.Main
local MPrompt = AliceHub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AliceHub:FindFirstChild('Drag')
...
local Notifications = AliceHub.Notifications

local SelectedTheme = AliceHubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AliceHubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AliceHub.Main.BackgroundColor3 = SelectedTheme.Background
	AliceHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AliceHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AliceHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AliceHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AliceHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AliceHub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		AliceHub.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		AliceHub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(AliceHub:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("AliceHub | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AliceHub | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AliceHub had an issue decoding the configuration file, please try delete the file and reopen AliceHub.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AliceHubLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AliceHub | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AliceHubLibrary:Notify({Title = "AliceHub Flags", Content = "AliceHub was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AliceHubLibrary.Flags) do
...
function AliceHubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AliceHub | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AliceHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show AliceHub'.", Duration = 7, Image = 4400697855})
		else
			AliceHubLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.AliceHubOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(AliceHubFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AliceHub Settings', 0, true)

	if TabList['AliceHub Settings'] then
		TabList['AliceHub Settings'].LayoutOrder = 1000
	end

	if Elements['AliceHub Settings'] then
		Elements['AliceHub Settings'].LayoutOrder = 1000
	end
...
function AliceHubLibrary:CreateWindow(Settings)
	if AliceHub:FindFirstChild('Loading') then
		if getgenv and not getgenv().AliceHubCached then
			AliceHub.Enabled = true
			AliceHub.Loading.Visible = true

			task.wait(1.4)
			AliceHub.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AliceHubCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AliceHubLibrary:Notify({Title = 'Build Mismatch', Content = 'AliceHub may encounter issues as you are running an incompatible interface version ('.. ((AliceHub:FindFirstChild('Build') and AliceHub.Build.Value) or 'No Build') ..').\n\nThis version of AliceHub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(AliceHubFolder) then
		makefolder(AliceHubFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AliceHub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AliceHub Interface Suite" then
		LoadingFrame.Version.Text = "AliceHub UI"
	end
	...
	if not Settings.DisableAliceHubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AliceHubLibrary:Notify({
					Title = "AliceHub Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(AliceHubFolder.."/Discord Invites") then
			makefolder(AliceHubFolder.."/Discord Invites")
		end

		if isfile and not isfile(AliceHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AliceHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AliceHub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AliceHubFolder.."/Key System") then
			makefolder(AliceHubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AliceHub | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AliceHub specific development.')
				end
			end
		end
		...
		if isfile and isfile(AliceHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AliceHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AliceHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AliceHubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AliceHub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AliceHub specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AliceHubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AliceHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AliceHub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AliceHub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AliceHub specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AliceHubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AliceHub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AliceHub specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AliceHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AliceHub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AliceHub specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AliceHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AliceHubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AliceHub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AliceHub specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AliceHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AliceHub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with AliceHub specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AliceHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AliceHubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AliceHubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AliceHub had an issue creating settings.') end
...
function AliceHubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AliceHubLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AliceHubLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AliceHub Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.AliceHubOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AliceHubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AliceHubLibrary:Notify({Title = "AliceHub Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AliceHubLibrary:Notify({Title = "AliceHub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AliceHub Configurations Error | '..tostring(result))
			AliceHubLibrary:Notify({Title = "AliceHub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = AliceHubLibrary:CreateWindow({
		Name = "AliceHub Example Window",
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using AliceHub may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AliceHub to get the key from
	...
	--AliceHubLibrary:Notify({Title = "AliceHub Interface", Content = "Welcome to AliceHub. These - are the brand new notification design for AliceHub, with custom sizing and AliceHub calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(AliceHubLibrary.Theme) do
...
task.delay(4, function()
	AliceHubLibrary.LoadConfiguration()
